...
const ingredients = {
  "Pantry Essentials": [
    "Butter", "Egg", "Garlic", "Milk", "Onion", "Sugar",
              ...
              "Cinnamon", "Ketchup", "Soy Sauce", "Mayonnaise", "Vegetable Oil"
...
"Arugula", "Asparagus", "Baby Corn", "Beetroot", "Bell Pepper", "Bitter Gourd", "Brussels Sprouts",
              ...
              "Sweet Potato", "Tomato", "Turnip", "Zucchini", "Chilli", "Capsicum" , "Ginger", "Drumstick","Curry Leaves","Coriander Leaves ","Mint Leaves","Broccoli" , "Bell Peppers", "Pumpkin","Jackfruit"],
    "Fruits":[
    ...
    "Dairy-Free & Meat Substitutes":["coconut milk", "almond milk", "soy milk", "cashew milk", "peanut milk", "oat milk",
      ...
      "Dairy and Eggs":[
        "paneer", "Bread", "Brown Bread", "chenna", "khoa", "malai", "curd", "hung curd", "buttermilk",
          ...
          "Legumes":["chickpea", "chana", "green chickpea", "chickpea sprouts", "chana dal", "kala chana",
...
const PantryApp = () => {
  ...
  const toggleIngredient = (item) => {
    setSelectedIngredients((prevSelected) =>
      prevSelected.includes(item)
        ? prevSelected.filter((ingredient) => ingredient !== item)
        : [...prevSelected, item]
    );
  };

  const filteredIngredients = Object.entries(ingredients).map(([category, items]) => {
  ...
  return (
    <div className="flex p-4 space-x-4 h-screen">
      {/* Left Side: Pantry Inventory */}
      <div className="w-1/3 bg-red-100 p-4 rounded-lg overflow-y-auto h-full">
        <h1 className="text-2xl center font-bold mb-4 ">Pantry</h1>
        <p className="text-gray-700 mb-2">You have {selectedIngredients.length} ingredient{selectedIngredients.length !== 1 ? "s" : ""} selected</p>
        <SearchBar onSearch={handleSearch} />
        <div className="overflow-y-auto max-h-[80vh] mt-2">
          {filteredIngredients.map(({ category, items }) => (
            items.length > 0 && (
              <div key={category} className="mb-4">
                ...
                <ul className="flex flex-wrap gap-2">
                  {items.map((item) => (
                    <li
                      key={item}
                      className={`px-4 py-2 cursor-pointer rounded-lg shadow ${selectedIngredients.includes(item) ? "bg-blue-300" : "bg-white hover:bg-gray-200"}`}
      ...
      <div className="w-2/3 sticky top-0 self-start">
        <ChatBot selectedIngredients={selectedIngredients} />
      </div>
    </div>
  );
};
